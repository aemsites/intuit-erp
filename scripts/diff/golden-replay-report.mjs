...
export function fieldRows(entry, scenario, outcome, deviations = { schemaVersion: 1, entries: [] }) {
  ...
  for (const location of ['envelope', 'properties', 'context', 'integrations']) {
    ...
    for (const [field, goldenValue] of locationFields(goldenPayload, location)) {
      ...
      const match = present && (spec.equalsPathname
        ? replayPathMatches(got, expected) : gatedMatch(spec, expected, got));
...
const metadataRows = (rows) => rows.filter((row) => row.bucket === 'gated'
  && row.category !== 'environment-session-context');

function metadataEventSummary(rows, status) {
  const metadata = metadataRows(rows);
  const exact = metadata.filter((row) => row.rawMatch).length;
  const accepted = metadata.filter((row) => ACCEPTED.has(row.category)).length;
  const bugs = metadata.filter((row) => row.category === 'stage-bug').length;
  const investigations = metadata.filter((row) => row.category === 'open-investigation').length;
  let basis = 'not-captured';
  if (status === 'captured' && !metadata.length) basis = 'unscored';
  else if (status === 'captured' && bugs + investigations) basis = 'failing';
  else if (status === 'captured' && accepted) basis = 'accepted-deviations';
  else if (status === 'captured') basis = 'exact';
  return {
    metadataTotal: metadata.length,
    metadataExact: exact,
    metadataAccepted: accepted,
    metadataBugs: bugs,
    metadataInvestigations: investigations,
    metadataParityBasis: basis,
  };
}
...
export function buildGoldenReplayReport({
  ...
  for (const outcome of state.outcomes) {
    ...
    const metadata = metadataEventSummary(rows, outcome.status);
    fields.push(...rows);
    events.push({
      scenarioId: scenario.scenarioId,
      page: scenario.page,
  ...
  const exactPathname = capturedEvents.filter((event) => replayPathMatches(event.pageCasId, event.page)).length;
  const pageCasId = {
    ...
    failures: capturedEvents.filter((event) => !replayPathMatches(event.pageCasId, event.page)).map((event) => event.scenarioId),
  };
  ...
  const scoredMetadataRows = metadataRows(fields);
  const exactMetadataRows = scoredMetadataRows.filter((row) => row.rawMatch);
  const acceptedMetadataRows = scoredMetadataRows.filter((row) => ACCEPTED.has(row.category));
  const bugMetadataRows = scoredMetadataRows.filter((row) => row.category === 'stage-bug');
  const investigationMetadataRows = scoredMetadataRows.filter((row) => row.category === 'open-investigation');
  const exactEvents = capturedEvents.filter((event) => event.metadataParityBasis === 'exact').length;
  const acceptedEvents = capturedEvents.filter((event) => event.metadataParityBasis === 'accepted-deviations').length;
  const failingEvents = capturedEvents.filter((event) => event.metadataParityBasis === 'failing').length;
  const unscoredEvents = capturedEvents.filter((event) => event.metadataParityBasis === 'unscored').length;
  const metadataParity = {
    totalFields: scoredMetadataRows.length,
    exact: {
      fields: exactMetadataRows.length,
      percent: percent(exactMetadataRows.length, scoredMetadataRows.length),
    },
    accepted: {
      fields: acceptedMetadataRows.length,
      percent: percent(acceptedMetadataRows.length, scoredMetadataRows.length),
      byClassification: Object.fromEntries([...ACCEPTED]
        .map((category) => [category, acceptedMetadataRows.filter((row) => row.category === category).length])),
      byPolicy: countsBy(acceptedMetadataRows, 'policy'),
    },
    adjusted: {
      fields: exactMetadataRows.length + acceptedMetadataRows.length,
      percent: percent(exactMetadataRows.length + acceptedMetadataRows.length, scoredMetadataRows.length),
    },
    bugs: {
      fields: bugMetadataRows.length,
      percent: percent(bugMetadataRows.length, scoredMetadataRows.length),
    },
    investigations: {
      fields: investigationMetadataRows.length,
      percent: percent(investigationMetadataRows.length, scoredMetadataRows.length),
    },
    events: {
      captured: capturedEvents.length,
      exact: exactEvents,
      accepted: acceptedEvents,
      failing: failingEvents,
      unscored: unscoredEvents,
      exactPercent: percent(exactEvents, capturedEvents.length),
      adjustedPercent: percent(exactEvents + acceptedEvents, capturedEvents.length),
    },
  };
  const categorizedMetadataFields = exactMetadataRows.length + acceptedMetadataRows.length
    + bugMetadataRows.length + investigationMetadataRows.length;
  if (categorizedMetadataFields !== scoredMetadataRows.length) {
    throw new Error('metadata parity breakdown does not account for every scored field');
  }
...
export function renderGoldenReplayHtml(report) {
  ...
  const acceptedClassifications = Object.entries(summary.metadataParity.accepted.byClassification)
